...
class Customize {
  ...
  async getAllData(req, res) {
    try {
      ...
      let DeliveredOrders = await orderModel.find({ status: "Delivered" });
    let Revenue = DeliveredOrders.reduce((total, order) => total + order.amount, 0);
      if (Categories && Products && Orders) {
        return res.json({ Categories, Products, Orders, Users,Revenue });
      }
    } catch (err) {
      console.log(err);
    }
