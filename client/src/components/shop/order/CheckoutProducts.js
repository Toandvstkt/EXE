...
export const CheckoutComponent = (props) => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup các tác vụ không đồng bộ hoặc hủy bỏ các subscription
    };
  }, [dispatch]);
          ...
          <div className="w-full order-first md:order-last md:w-1/2">
              ...
              <Fragment>
                ...
                >
                  ...
                  <Box>
                    <Typography fontWeight="medium">Choose a way to pay</Typography>
                    <Card
                      sx={{
                        mt: 2,
                        mb: 2,
                    ...
                    >
                      {selectedOption !== 'bank' && (
                        <CardActionArea sx={{ borderRadius: 0 }} onClick={() => handleOptionClick('cod')}>
                          <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <PaidIcon color="primary" fontSize="small" />
                            <Typography fontWeight="medium">Ship COD</Typography>
                          </CardContent>
                        </CardActionArea>
                      )}
                      {selectedOption !== 'cod' && (
                        <CardActionArea
                          sx={{ borderTop: selectedOption == '' ? '1px solid #B5B5B5' : 'none', borderRadius: 0 }}
              ...
              >
                <div className="md:flex md:items-center md:space-x-4">
                  ...
                  <div className="md:ml-6 font-semibold text-gray-600 text-sm">
                    Price : {product.pPrice}.000 vnđ{" "}
                  </div>
                  <div className="md:ml-6 font-semibold text-gray-600 text-sm">
                    Quantity : {quantity(product._id)}
                  </div>
                  <div className="font-semibold text-gray-600 text-sm">
                    Subtotal : {subTotal(product._id, product.pPrice)}.000 vnđ
                  </div>
                </div>
              </div>
            );
