...
import { getProductByType } from "../../admin/products/FetchApi";
...
const ProductComponent = (props) => {
  ...
  const fetchData = async () => {
    ...
    try {
      let responseData = await getProductByType(pType);
      setTimeout(() => {
            ...
            <Fragment key={item._id}>
              <div className="relative m-2">
                ...
                <div className="absolute top-0 right-0 mx-2 my-2 md:mx-4">
                  <svg
                    onClick={() => isWishReq(item._id, setWlist)}
                  ...
                  <svg
                    onClick={() => unWishReq(item._id, setWlist)}
                    className={`${!isWish(item._id, wList) && "hidden"
